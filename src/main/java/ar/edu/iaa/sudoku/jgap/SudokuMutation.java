...
import ar.edu.iaa.sudoku.domain.Cell;
 ...
 *
 * This SudokuMutation supports both fixed and dynamic mutation rates. A fixed
...
public class SudokuMutation extends BaseGeneticOperator implements Configurable {
	...
	/**
	 * Calculator for dynamically determining the mutation rate. If set to null
	 * the value of m_mutationRate will be used. Replaces the previously used
	 * boolean m_dynamicMutationRate.
	 */
	private IUniversalRateCalculator m_mutationRateCalc;

	private SudokuMutationConfigurable m_config = new SudokuMutationConfigurable();

	private Sudoku sudokuToEvaluate;

	/**
	 * Constructs a new instance of this SudokuMutation without a specified
	 ...
	 * @since 1.0
	 */
	public SudokuMutation() throws InvalidConfigurationException {
	 ...
	 * @since 3.0
	 */
	public SudokuMutation(final Configuration a_conf)
			throws InvalidConfigurationException {
		super(a_conf);
	...
	/**
	 * Constructs a new instance of this SudokuMutation with a specified
	 ...
	 * @since 1.1
	 */
	public SudokuMutation(final Configuration a_config,
	...
	/**
	 * Constructs a new instance of this SudokuMutation with the given mutation
	 * rate.
	 *
	 * @param a_config
	 *            the configuration to use
	 ...
	 *            the pool of chromosomes that have been mutated
	 *
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	 * @since 1.0
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < genes1.length; j++) {
				if (m_mutationRateCalc != null) {
					// If it's a dynamic mutation rate then let the calculator
					// decide
					// whether the current gene should be mutated.
					// --------------------------------------------------------------
					mutate = m_mutationRateCalc.toBePermutated(chrom, j);
				} else {
					// Non-dynamic, so just mutate based on the the current
					// rate.
				...
				if (mutate) {
					...
					for (int r = 0; r < sudokuToEvaluate.getCellToMutateSize() /2; r++) {
						Cell mutateCell = sudokuToEvaluate
								...
								//.getRepeatCell(r);
						int position = sudokuToEvaluate
								.getCellValue(mutateCell);
						int value = sudokuToEvaluate
								.getRepeatValueSolution(mutateCell);
						genes[position].setAllele(value);
					}
					// mutateGene(genes[position], generator);
	 ...
	 * @param a_generator
	 *            the generator delivering the amount of mutation
	 *
	 * @author Klaus Meffert
	 * @since 1.1
	 */
	...
	/**
	 * Sets the MutationRateCalculator to be used for determining the strength
	 * of mutation.
	 *
	 * @param a_mutationRateCalc
	 *            MutationRateCalculator
	 *
	 * @author Klaus Meffert
	 * @since 1.1
	 */
	public void setMutationRateCalc(
			final IUniversalRateCalculator a_mutationRateCalc) {
	...
	/**
	 *
	 * @param a_mutationRate
	 *            new rate of mutation, expressed as the denominator of the 1 /
	 *            X fraction. For example, 1000 would result in 1/1000 genes
	 *            being mutated on average. A mutation rate of zero disables
	 *            mutation entirely
	 *
	 * @author Klaus Meffert
	 ...
	 * true if and the argument is an instance of this class and is equal wrt
	 * the data.
	 *
	 * @param a_other
	...
	public int compareTo(Object a_other) {
		...
		SudokuMutation op = (SudokuMutation) a_other;
		if (m_mutationRateCalc == null) {
	...
	public void setSudoku(Sudoku aSudoku) {
		this.sudokuToEvaluate = aSudoku;
	}
	...
	class SudokuMutationConfigurable implements java.io.Serializable {
		 ...
		 * that, on average, 1 / 1000 genes would be mutated. A value of zero
